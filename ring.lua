-- ==========================================
-- HỆ THỐNG GIAO DIỆN (ROCK FRUIT - V7 TỐI THƯỢNG)
-- ==========================================
...
-- LỚP OVERLAY (Bấm ra ngoài để đóng List)
...
Title.Text = "Yui HUB - ROCK FRUIT v7 (Pro Mobile)"
...
-- ==========================================
-- HỆ THỐNG DROPDOWN NỔI ĐÈ (TỰ ĐÓNG KHI BẤM RA NGOÀI)
-- ==========================================
...
local function CreateFloatingDropdown(parentBtn, titleText, getItemsFunc, onSelectFunc)
    ...
    DropFrame.BorderColor3 = Color3.fromRGB(0, 150, 255)
    DropFrame.ZIndex = 10
    ...
    parentBtn.MouseButton1Click:Connect(function()
        for _, drop in pairs(ActiveDropdowns) do drop.Visible = false end -- Đóng các list khác
        ...
        for _, item in ipairs(items) do
            ...
            -- Chức năng chọn nhiều (Tick xanh)
            local isSel = false
        ...
        -- Tính toán vị trí xổ xuống dưới nút bấm
...
-- Hàm Teleport Chuẩn xác 100% (Sửa lỗi kẹt)
local function TP(targetObj)
    if not targetObj then return end
    local char = Player.Character
    if char and char:FindFirstChild("HumanoidRootPart") then
        if targetObj:IsA("Model") then
            char:PivotTo(targetObj:GetPivot() * CFrame.new(0, 5, 0))
        elseif targetObj:IsA("BasePart") then
            char.HumanoidRootPart.CFrame = targetObj.CFrame * CFrame.new(0, 5, 0)
        end
    end
end

-- Fuzzy Search tìm mục tiêu ẩn
local function FindAndTP(name)
    local lName = string.lower(name)
    for _, v in pairs(workspace:GetDescendants()) do
        if string.find(string.lower(v.Name), lName) and (v:IsA("Model") or v:IsA("BasePart")) then
            TP(v); return true
        end
    end
    return false
...
local MobDropBtn = CreateButton(SecMob, "Chọn Quái (Sổ Xuống) ▼", function() end)
CreateFloatingDropdown(MobDropBtn, "Mobs", function()
    ...
    for _, v in pairs(folder:GetChildren()) do
        if v:FindFirstChild("Humanoid") and v.Name ~= Player.Name and not table.find(mobs, v.Name) then table.insert(mobs, v.Name) end
    end
    return mobs
end, function(itemName, isAdded)
    if isAdded then table.insert(_G.SelectedMobs, itemName)
    else for i, v in ipairs(_G.SelectedMobs) do if v == itemName then table.remove(_G.SelectedMobs, i) break end end end
end)

local SecWep = CreateSection(MainLeft, "Auto Equip Weapon")
local WepDropBtn = CreateButton(SecWep, "Chọn Vũ Khí (Sổ Xuống) ▼", function() end)
CreateFloatingDropdown(WepDropBtn, "Weapons", function()
    ...
    if isAdded then table.insert(_G.SelectedWeapons, itemName)
    else for i, v in ipairs(_G.SelectedWeapons) do if v == itemName then table.remove(_G.SelectedWeapons, i) break end end end
end)
...
-- [3] TAB TELEPORT (Đã Fix Teleport Hoàn Hảo)
...
local NpcList = {"Quest 1", "Sword Dealer", "Fruit Dealer", "Blacksmith"}
local NpcBtn = CreateButton(SecNpc, "Đích: Quest 1", function() end)
local NpcIdx = 1
...
-- Hàng 3 lựa chọn x5 x10 x15
local ChestRow = Instance.new("Frame", SecChest)
ChestRow.Size = UDim2.new(1, -10, 0, 26); ChestRow.BackgroundTransparency = 1
local LayoutChestRow = Instance.new("UIListLayout", ChestRow)
LayoutChestRow.FillDirection = Enum.FillDirection.Horizontal
LayoutChestRow.Padding = UDim.new(0, 5)
...
RunService.RenderStepped:Connect(function()
    -- XỬ LÝ FLY CHUẨN CAMERA (Bình thường, không bị dọc thẳng đứng)
    if _G.Fly and Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
        ...
        -- Tính hướng bay theo Camera + Joystick của Mobile
        local moveDir = hum.MoveDirection
        if moveDir.Magnitude > 0 then
            -- Tính toán trục hướng tới theo góc nhìn camera
            local pitch = cam.CFrame.LookVector.Y
            local velocity = moveDir * _G.FlySpeed
            -- Nếu đang đi tới, cho phép bay lên/xuống theo góc ngẩng camera
            local forwardness = moveDir:Dot(Vector3.new(cam.CFrame.LookVector.X, 0, cam.CFrame.LookVector.Z).Unit)
            if forwardness == forwardness then -- Kiểm tra NaN
                velocity = Vector3.new(velocity.X, forwardness * pitch * _G.FlySpeed, velocity.Z)
            end
...
task.spawn(function()
    while task.wait(0.1) do
        ...
        -- LOGIC AUTO FARM LEVEL (Cày cấp thông minh)
        if _G.AutoFarmLevel and Player:FindFirstChild("leaderstats") and Player.leaderstats:FindFirstChild("Level") then
            ...
            -- Tự động đổi mục tiêu theo Level
            if myLevel < 1000 then
                _G.SelectedMobs = {"Bandit"} -- Tên quái Quest 1
                -- Lệnh nhận Quest tự động tại đây (Ví dụ FireServer quest 1)
            else
                _G.SelectedMobs = {"Boss"} -- Tên quái Quest 2
            end
            _G.AutoFarm = true -- Bật Farm
        end
        
        -- LOGIC AUTO MUA RƯƠNG (Chest) & RANDOM FRUIT
        if _G.AutoBuyChest then
            -- Mua rương liên tục theo _G.ChestAmount (5,10,15)
            -- Code FireServer Mua Rương của Game
        end
        if _G.AutoRandomFruit then
            -- Code FireServer Random Trái Ác Quỷ
        end
